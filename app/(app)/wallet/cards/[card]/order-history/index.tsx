...
export default function OrderHistory() {
  ...
  return (
    ...
    >
      ...
      >
        {/* no order history */}
        {(!mockCard?.orderHistory || mockCard?.orderHistory?.length === 0) && (
          <NoOrderHistory />
        )}
        {/* <NoOrderHistory /> */}
        ...
        {/* order summary */}
        <View style={{ gap: 12 }}>
          {mockCard?.orderHistory?.map((item, idx) => (
            <OrderSummary
              key={item.orderId}
              title={item.title}
              date={item.date}
  ...
  return (
    ...
    >
      ...
      <CommonPageTitleSection
        ...
        title={"No History"}
        description="Recent orders from uKnomi participating restaurants will appear here"
      />
    </View>
  );
}
